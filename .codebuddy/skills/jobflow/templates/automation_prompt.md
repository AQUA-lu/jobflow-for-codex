...
Before making decisions, run the local ledger validation command. Treat validation errors as blockers and review warnings before enabling recurring automation.
...
9. Use `upsert_ledger.py` to write or update a ledger row only after visible platform success state.
