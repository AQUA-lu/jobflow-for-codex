...
JobFlow is a local job-search workflow for Codebuddy. Use it when the user wants help finding jobs, screening roles, contacting recruiters, reviewing recruiter messages, sending resumes when clearly requested, maintaining an application ledger, setting up recurring job-search automations via Codebuddy's automation system, or generating job-search reports.

## Core Rules

- Treat this as an assistant workflow, not an unattended mass-application bot.
- Keep private user data in the user's workspace, never inside this skill.
